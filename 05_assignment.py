...
def exercise01():
    ...
    df = pd.DataFrame(np.array([[1, 2, 3, 4], [5, 6, 7, 8], [7, 8, 9, 10]]))
...
def exercise08():
    ...
    url_content = res.content.decode('utf-8')
    df = pd.read_csv(io.StringIO(url_content))
    row_count = len(df)
...
def exercise13(n):
    ...
    s = pd.Series(np.random.randint(0, n, n),
                  index=pd.date_range(start="1/1/2010", periods=n))
    plt.plot(s.cumsum())
    plt.show()
    # ------ Place code above here /\ /\ /\ ------
    return s
